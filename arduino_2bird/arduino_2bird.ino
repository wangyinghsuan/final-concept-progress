...
//if(X==0 && Y==0 )   for(int fadeValue = 0 ; fadeValue <= 180; fadeValue +=0.5) { 
 ...
 // delay(300);                     
 // } else
if(X < 1023 && X >= 0 ) value1 = value1 + (map(X,0,1023,160,0) - value1) * 0.2;

 // if(Y < 1023 && Y >= 0 )value2 = map(Y,0,1023,50,160);
     if(Y < 1023 && Y >= 0 ) value2 = value2 + (map(Y,0,1023,10,180) - value2) * 0.2;

if(X < 1023 && X >= 0 ) value3 = value3 + (map(X,0,1023,0,160) - value3) * 0.2;

 // if(Y < 1023 && Y >= 0 )value2 = map(Y,0,1023,50,160);
     if(Y < 1023 && Y >= 0 ) value4 = value4 + (map(Y,0,1023,10,160) - value4) * 0.2;
